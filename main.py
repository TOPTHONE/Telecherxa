...
def start(message):
	...
	if f == SUDO:
		...
		rr.add(me,he,de)
		rr.add(ch)
		bot.send_message(message.chat.id,text="Ù‡Ø°Ø§ Ø§Ù„Ø¨ÙˆØª Ù…Ø¯ÙÙˆØ¹ ÙˆÙ„ÙŠØ³ Ù„Ùƒ\n Ù„Ù„ØªÙØ¹ÙŠÙ„ Ø±Ø§Ø³Ù„",reply_markup=rr)
...
def masg(call):
	...
	if call.data =="Y_U_8":
		...
		v.add(K,J,O,I,S)
		v.add(B)
		bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text="Ø§Ø®ØªØ± Ù…Ù† Ø§Ù„Ù‚Ø§Ø¦Ù…Ù‡ Ø¨Ø§Ù„Ø§Ø³ÙÙ„ .",reply_markup=v)
	elif call.data =="F1":
		...
		e.add(f,c,z)
		e.add(bc)
		bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text="Ø§Ø®ØªØ± Ù…Ù† Ø§Ù„Ù‚Ø§Ø¦Ù…Ù‡ Ø¨Ø§Ù„Ø§Ø³ÙÙ„ .",reply_markup=e)
	elif call.data =="F100":
		e = types.InlineKeyboardMarkup(row_width=2)
